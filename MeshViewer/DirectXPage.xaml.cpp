...
#include "Loaders\FilmboxLoader.h"
	...
	m_coreInput(nullptr)
{
	...
	m_main = std::unique_ptr<MeshViewerMain>(new MeshViewerMain(m_deviceResources));
...
void DirectXPage::Load(ILoader ^ loader)
{
	FileOpenPicker^ picker = ref new FileOpenPicker();
	picker->FileTypeFilter->Append(".fbx");
	auto fileTask = create_task(picker->PickSingleFileAsync());
	fileTask.then([this, loader](Windows::Storage::StorageFile^ file)
	{
		if (file != nullptr) {
...
void DirectXPage::Browse_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	Load(ref new Loaders::FilmboxLoader());
}
